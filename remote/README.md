...
Windows 版托盘程序提供与菜单栏一致的远程控制入口，后端命令复用
`launcher/win/remote-backend.mjs`。

- 「启动远程」只开启 daemon，适合手机已经配对、只需要恢复连接的场景。
- 「扫码配对手机」会按需先启动远程，再显示二维码，适合首次配对或新增设备。
- 「停用远程」会删除计划任务，并兜底结束当前安装目录下残留的 daemon 进程。
- 退出托盘时会先停用远程，避免用户以为已关闭但后台仍在运行。
