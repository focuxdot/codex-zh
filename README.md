# Codex-ZH · Codex 中文版（汉化 · 国内可用）

Codex-ZH 是 OpenAI Codex 的中文汉化定制版，面向国内 Windows 用户：默认中文界面，内置中转站（Wokey / OpenRouter / 自定义）配置向导，免改 `config.toml` 就能在国内 API Key 场景下启用 Browser、Chrome、Computer Use 等本地能力。

> 关键词：Codex 中文版、Codex 汉化、Codex 国内怎么用、Codex 中转站配置、Codex Windows 安装包、Browser / Computer Use 地区不可用。
...
如果你通过 CC-switch 等外部工具切换模型，不希望 Codex-ZH 启动时打开中转站配置向导，可以在弹窗里勾选“以后不再显示这个配置弹窗”并点击“跳过本次”。启动器也支持 `CodexZhLauncher.exe --skip-config` 跳过本次配置向导；需要重新打开配置时，使用开始菜单里的“Codex 中转站配置”。
