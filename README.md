...
手机远程内置在 macOS 版里：点菜单栏的 Codex 图标 →「扫码配对手机」，手机扫码就连上了（首次扫码即自动开启远程，无需先手动启用）；不用则完全不运行。Windows 版的集成在排期中。开发者也可以直接从源码运行，见 [remote/README.md](remote/README.md)（含中继自部署文档）。
...
4. 想用手机远程：点菜单栏的 Codex 图标 →「扫码配对手机」，手机扫码即连上（首次扫码自动开启远程）。
